...
const OrderDetail=async (db_name,order_id)=>{
    ...
    var  select_query="select  CONCAT(usr.firstname,usr.lastname) AS customer_name, "+
    " ors.drop_off_date,sp.name as supplier_name,ors.agent_verification_code,ors.have_coin_change,ors.created_on,ors.schedule_date as delivered_on,ors.schedule_date as delivery_date,usr.mobile_no as customer_phone_number,usr.user_image as customer_image ,CAST(usr.id as CHAR(50)) as customer_id,"+
    ...
    " ors.urgent,ors.net_amount,ors.delivery_charges,ors.handling_supplier,"+
...
async function RunTimeAgentConnection(data){
        var password=await require('../util/Universal').getDecryptData(data.password)
...
const GetAgentAvailability=async (secret_key,token)=>{
    return new Promise((resolve,reject)=>{
      request.get({
          headers: {
                'secret_key':secret_key,
          ...
          url: config.get("agent.api_link")+config.get("agent.get_agent_availability"),
          method: "GET"
        }, function(error, response, body) {
            logger.debug("===Body!==",error,"=====MES=",body);
...
const ridesRegisteration=async (baseUrl,data,dbSecretKey)=>{
    console.log("=====Rides=Registratio=inputData==",baseUrl,dbSecretKey,data,config.get("server.rides.api_link")+config.get("server.rides.registeration"));
    return new Promise((resolve,reject)=>{
        if(dbSecretKey!=""){
            request.post({
                headers: {
            ...
            request.post({
                ...
                json: true
                  ...
                  logger.debug("===ridesRegisteration!==",error,"=====MES=",body);
                   resolve()
              })
        }
