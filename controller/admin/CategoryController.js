...
class permision{
    /**
     * @description used for add/update an category permission to admin
     * @param {*Object} req 
     * @param {*Object} res 
     * @param {*Function} next 
     */
    static async assignment(req,res,next){
       try{
           let _promise=[];
           let _categoryIds=req.body.categoryIds;
           let _adminId=req.body.adminId || req.user.id;  
           _promise.push(await model.admin.permissionaction.unassigned(_adminId));
           _promise.push(await model.admin.permissionaction.assigned(_categoryIds,_adminId));
           await Promise.all(_promise).then((data)=>{
     ...
     /**
     * @description used for listing an assigned data to admin
     * @param {*Object} req 
     * @param {*Object} res 
     * @param {*Function} next 
     */
    static async assignmentList(req,res,next){
        try{
            let _promise=[];
            let _categoryIds=req.body.categoryIds;
            let _adminId=req.query.adminId || req.user.id;  
            _promise.push(await model.admin.permissionaction.list(_adminId));
            await Promise.all(_promise).then((process)=>{
                    logger.debug("==process======>>",process)
                  sendResponse.sendSuccessData(process[0], constant.responseMessage.SUCCESS, res, constant.responseStatus.SUCCESS);
            }).catch((Err)=>{
               sendResponse.somethingWentWrongError(res)
            })
        }
