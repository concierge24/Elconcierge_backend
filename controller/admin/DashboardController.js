...
function getTotalActiveAgents(dbName){
...
const getTotalActiveOrdersAgents =  (dbName)=>{
    return new Promise(async(resolve,reject)=>{
        try{
            ...
            from  cbl_user join cbl_user_orders o on o.user_id = cbl_user.id where deleted_by=0  and is_available=1 and is_active=1
...
const supplierProfitAfterTaxCommissionAccDate=(dbName,supplierBranchId,start_date,end_date)=>{
    ...
    return new Promise(async (resolve,reject)=>{
        ...
        let bQuey="SELECT `id` from orders where DATE(created_on) >='" + start_date + "' and DATE(created_on) <= '" + end_date + "' and supplier_branch_id = ? and status = 5 "
        ...
        // let orderData=await ExecuteQ.Query()
...
const supplierProfitAfterTaxCommissionV1=(dbName,startDate,endDate,supplierBranchId)=>{
    ...
    return new Promise(async (resolve,reject)=>{
        ...
        try{
        let order_ids=[],total_supplier_profit=0;
        let sql ="SELECT `id`,`created_on`,`handling_admin`,`handling_supplier`,`delivery_charges`,`net_amount`,`supplier_commision` from orders where supplier_branch_id = ? and status = 5 and DATE(created_on)>='"+startDate+"' and DATE(created_on)<='"+endDate+"' "
        let orderData= await ExecuteQ.Query(dbName,sql,[supplierBranchId])
...
const Supplier_get_orders_count = (db_name, supplierId) => {
    ...
    return new Promise((resolve, reject) => {
        ...
        sqlQuery += "count(IF(o.status=5,1,NULL)) as completed_order_count, ";
...
const TotalOrderRevenueAccDate=(start_date,end_date,db_name,is_single_vendor)=>{    
    return new Promise((resolve,reject)=>{
        ...
        " where DATE(created_on) >= ? and DATE(created_on)<=? AND status IN (5) GROUP BY created_at ORDER BY DATE(created_on) ASC"
       var st= multiConnection[db_name].query(sqlQuery,[start_date,end_date],(err,data)=>{
        logger.debug(st.sql)    
        if(err){
                reject("Something Went Wrong")
...
const TotalOrderRevenue = (db_name,is_single_vendor)=>{
    return new Promise(async (resolve,reject)=>{
            try{
                ...
                " where status IN (5) ORDER BY DATE(created_on) ASC"
...
const TotalOrderRevenueByDate = (db_name,is_single_vendor,startDate,endDate)=>{
    return new Promise(async (resolve,reject)=>{
        ...
        " where status IN (5) and DATE(created_on)>='"+startDate+"' and DATE(created_on)<='"+endDate+"' ORDER BY DATE(created_on) ASC"
        let data=await ExecuteQ.Query(db_name,sqlQuery,[]);
        let cancelationDeductionAmount=0;
        let deliveryChargeAmount=0;
        let promoBearAmount=0;
...
const get_orders_count = (db_name)=>{
    return new Promise((resolve,reject)=>{
        ...
        sqlQuery += "count(IF(status=8,1,NULL)) as cancel_order_count from orders  join order_prices op on op.order_id = orders.id  " 
...
const get_register_supplier = (db_name)=>{
    return new Promise((resolve,reject)=>{
        var sqlQuery = "select count(*) as supplier_count from supplier where is_deleted=0";
...
const get_total_categories = (db_name)=>{
    return new Promise((resolve,reject)=>{
        var sqlQuery = "select count(*) as category_count from categories where is_deleted=0 and parent_id=0 and id!=1";
        var stmt = multiConnection[db_name].query(sqlQuery,[],(err,data)=>{
            logger.debug(stmt.sqlQuery,err)
            if(err){
                reject("something went wrong");
...
const get_total_product = (db_name)=>{
    return new Promise((resolve,reject)=>{
        var sqlQuery = "select count(*) as product_count from product where is_deleted=0";
        var stmt = multiConnection[db_name].query(sqlQuery,[],(err,data)=>{
            logger.debug(stmt.sqlQuery,err)
            if(err){
                reject("something went wrong");
