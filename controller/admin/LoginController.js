...
const Login=async (req,res)=>{
    try{
        ...
        let user_data=await ExecuteQ.Query(req.dbName,"select `id`,`is_active`,`email`,`password`,`fcm_token`,`is_superadmin`,`user_created_id` from admin where `email`=?",[email]);

        let _categoryPermission=[]

        if(user_data && user_data.length>0){

            let encrypted_password = md5(password);
            logger.debug("=======encrypted_password==>>",encrypted_password);
            _categoryPermission=await model.admin.permissionaction.list(user_data[0].id) 
            ...
            if(encrypted_password==user_data[0].password){
                ...
                if (is_single_vendor) {
                    ...
                    response_data = {
                        ...
                        "categoryPermission": _categoryPermission,
