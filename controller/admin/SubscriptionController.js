...
const createSubscriptionPlan = async (req,res)=>{
    try{
        ...
        let plan_id = await createPlan(req.dbName,name,type,price,res);
...
const supplierListPlans = async (req,res)=>{
    try{
        ...
        let query2 = "select * from supplier_subscription where supplier_id=? and status = ? "
...
const getAllPlansFromDb = async (dbName,limit,offset)=>{
    return new Promise(async(resolve,reject)=>{
        try{
            ...
            let query = "select * from subscription_plans where is_deleted=0  limit ?,?"
...
const getAllPlansFromDbForSupplier = async (dbName,limit,offset)=>{
    return new Promise(async(resolve,reject)=>{
        try{
            ...
            let query = "select * from subscription_plans where is_deleted=0 and is_block=0 limit ?,?"
            let params = [offset,limit]
            let plans_data = await ExecuteQ.Query(dbName,query,params)
            let plans = []
            if(plans_data && plans_data.length>0){
...
const subscriptionCreateWebhook = async(req,res)=>{
    try{
        ...
        let status =m_data.status
        let subscription_id = sub_data.object.id
        // let customer_id = m_data.metadata.customer_id
        let current_period_end = m_data.current_period_end
        let current_period_start = m_data.current_period_start
        let created = m_data.created
        let stripe_plan_id = m_data.plan.id
        let dbName = m_data.metadata.dbName
        let supplierId = m_data.metadata.supplier_id
        let plan_id = m_data.metadata.plan_id
...
const buySupplierSubscription = async(req,res)=>{
    try{
        ...
        current_period_start,current_period_end,payment_source,reciept_url)
        values(?,?,?,?,?,?,?,?)`

        let params = [supplierId,"active",subscription_id,
            plan_id,current_period_start,
            current_period_end,payment_source,reciept_url]
...
function suppliersSubscriptions(dbName,limit,offset,period,input,
    ...
    return new Promise(async(resolve,reject)=>{
        ...
        if(fromDate!=="" && toDate!==""){
            dateFilter = " DATE( ss.created_at ) >= '"+fromDate+"' AND DATE( ss.created_at ) <=  '"+toDate+"' "
        }
        ...
        query += "where sp.plan_type like '%"+period+"%' "+filterSql+" group by ss.id limit ?,?"     
