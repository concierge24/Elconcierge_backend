...
const listSuppliers = async (req,res)=>{
    ...
    try{
        ...
        let language_id=req.query.language_id || 14;
        ...
        let getRegisterSupplier = await  getRegisterSuppliers(req.dbName,res,limit,
            ...
            is_stripe_connected,sequence_wise,language_id)
       
            logger.debug("======getRegisterSupplier======++",getRegisterSupplier)
        ...
        let final = await clubDataForRegSupplier(res,result)
...
const listAllSuppliers = async (req,res)=>{
    try{
        ...
        let supplierData=await queryModel.supplierModel.supplierListWithoutPagination(["email","id","name"],req.dbName,[]);
...
async function getRegisterSuppliers(dbName,res,limit,
    ...
    return new Promise(async(resolve,reject)=>{
        try{
            ...
            " s.stripe_account, s.federal_number,s.user_service_charge,s.mobile_number_1,s.speciality,s.nationality,s.facebook_link,s.linkedin_link,s.brand,s.speciality,s.description,s.is_active,s.pricing_level,s.commission,s.pickup_commission from supplier s join supplier_ml sml on sml.supplier_id = s.id join supplier_branch sb on sb.supplier_id = s.id where s.is_active=? "+stripeFilterCheck+" "+is_out_network_check+"  "+country_code_query+" and sml.language_id="+language_id+" and sb.is_head_branch=1 and (s.name like '%"+search+"%' or s.email like '%"+search+"%') "+order_by_query+" "+limitSql+"  ";
            ...
            if(parseInt(sequence_wise)!==1){
                if (order_by == 1) {
                    ...
                    finalResult = _.sortBy(finalResult,'id')
                }
            }
        ...
        //    logger.debug("======SuplData======",suppliers_data); 
            if(suppliers_data && suppliers_data.length){
                for(const [index,i] of suppliers_data.entries()){
                    ...
                    json_data.mobile_number_1=i.mobile_number_1
...
function clubDataForRegSupplier(res,result) {
...
const addSupplierUpdationRequest = async (req, res) => {
    ...
    try {
        let {supplierName, supplierEmail,
            ...
            supplier_id,updationRequestId} = req.body
        ...
        if(updationRequestId!==undefined && updationRequestId!==0){
            ...
            if(req.files.logo){
                ...
                let query = ` update supplier_updation_requests set name=? where id=? `;
                await Execute.Query(req.dbName,query,[logo]);            }

            if(req.files.supplier_image){
                let supplier_image = await uploadMgr.uploadImageFileToS3BucketNew(req.files.supplier_image)
                let query = ` update supplier_updation_requests set name=? where id=? `;
                await Execute.Query(req.dbName,query,[supplier_image]);
            }

            var query = ` update supplier_updation_requests set name=?,
                ...
                facebook_link=?,linkedin_link=?,brand=? where id=? `;
            ...
            if(req.files.logo){
                 logo = await uploadMgr.uploadImageFileToS3BucketNew(req.files.logo)  
            }
            if(req.files.supplier_image){
                 supplier_image = await uploadMgr.uploadImageFileToS3BucketNew(req.files.supplier_image)
            }

            var query = ` insert into supplier_updation_requests(name,
                email,mobile_number_1, 
                address,latitude,
                longitude,commission,
                pickup_commission,country_code,
...
const approveSupplierUpdationRequest = async (req, res) => {
    try {
        let {updationRequestId} = req.body
        ...
        let {name,
            ...
            await Execute.Query(req.dbName,
                "update supplier_updation_requests set  update_request_approved=1 where id=?",[updationRequestId])
...
const getSupplierUpdationRequests = async (req, res) => {
    ...
    try {
        ...
        var query = ` select * from supplier_updation_requests limit ?,? `;
...
const UpdateSupplierAvailability = async (req, res) => {
    try {
        ...
        logger.debug("-------------------req.dbname==========",req.dbName)
        ...
        */
        ...
        await updateSupplierAvailabilityAndTimings(req.dbName,supplier_id,
            supplier_timings,date_ids,date_order_type,supplier_location_id);
    ...
    return new Promise(async(resolve,reject)=>{
        try{
            ...
            if(weeks_data && weeks_data.length>0){
                ...
                for(const [index,i] of weeks_data.entries()){
                let query = "update supplier_availability set status=? where id=? and supplier_id=?";
                let params = [i.status,i.id,supplier_id];
            ...
            if(supplier_timings && supplier_timings.length>0){
                for(const [index,i] of supplier_timings.entries()){
                    if(i.id!==undefined && i.id!==""){
                        ...
                        // await ExecuteQ.Query(dbName,`delete from supplier_slot_timings where supplier_id=? and supplier_location_id=?`,[supplier_id,supplier_location_id]);
...
const supplierSlotTimings = (dbName, supplier_id,date_order_type,supplier_location_id) => {
    return new Promise(async (resolve, reject) => {
        try {
            let query = "select id,supplier_id,status,start_time,end_time,date_id,day_id,`offset`,date_order_type,supplier_location_id,price,quantity from supplier_slot_timings ";
                query += "where supplier_id = ? and date_order_type=? and supplier_location_id=? ";
...
const getSupplierSlots = async (req, res) => {
    ...
    try {
        ...
        let interval = slotsInterval>0?slotsInterval:30;
        ...
        let AvailableSlots = await getAvailSlotsWithOrderType(req.dbName,supplier_id,
            ...
            let _bookedData=await ExecuteQ.Query(req.dbName,"select booked_slots from supplier_booked_slots where booked_date=?",[date]);
            if(_bookedData && _bookedData.length>0){
                for (const [index,i] of _bookedData.entries()){
                    _bookedSlots.push(i.booked_slots)
                }
            }
            ...
            if(req.path=="/user/list_supplier_slots/v1"){
                ...
                sendResponse.sendSuccessData({slots:slots,availableTables:availableTables}, constant.responseMessage.SUCCESS, res, constant.responseStatus.SUCCESS);
                ...
                slots = _.uniq(slots);
                slots = slots.filter(val => !_bookedSlots.includes(val));
    ...
    try {
        ...
        if(parseInt(status)==1){
            ...
            let day_id =  moment(booked_date).day();
             supplier_id = slotData[0].supplier_id
        ...
        if(parseInt(status)==1){
            ...
            message = "your table booking is rejected"
        }else{
            message = "your table booking "
        }
            ...
            let table_book_mac_theme = await ExecuteQ.Query(dbName,
                            ...
                            if(error){
                             ...
                             web_request(options1,async function (error1, response1, body1) {
                                        ...
                                        await bookTable(req.dbName,branch_id,user_id,table_id,
                                            ...
                                            zelle_receipt_url,1,seating_capacity);
                                        sendResponse.sendSuccessData({},constant.responseMessage.SUCCESS, res, 200);
                                ...
                                console.log("==Payment===ERR!==>>",err);
                                    if(err){
                                        ...
                                        await bookTable(req.dbName,branch_id,
                                            ...
                                            payment_source,zelle_receipt_url,1,seating_capacity);
                             sendResponse.sendSuccessData({},constant.responseMessage.SUCCESS, res, 200);
                ...
                sendResponse.sendSuccessData({},constant.responseMessage.SUCCESS, res, 200);
                 ...
                 sendResponse.sendSuccessData({},constant.responseMessage.SUCCESS, res, 200);
                    ...
                    await bookTable(req.dbName,branch_id,
                        user_id,table_id,slot_id,schedule_date,
                        schedule_end_date,supplier_id,amount,payment_source,zelle_receipt_url,1);
                sendResponse.sendSuccessData({},constant.responseMessage.SUCCESS, res, 200);
                ...
                sendResponse.sendSuccessData({},constant.responseMessage.SUCCESS, res, 200);
                    ...
                    await bookTable(req.dbName,branch_id,
                        user_id,table_id,slot_id,schedule_date,
                        schedule_end_date,supplier_id,amount,payment_source,zelle_receipt_url,1,seating_capacity);
                    sendResponse.sendSuccessData({},constant.responseMessage.SUCCESS, res, 200);
                                    ...
                                    instance.createPayment(cbrequest, async  function (error, data, response) {
                                        if (error) {
                                            ...
                                            await bookTable(req.dbName,branch_id,
                                                ...
                                                amount,payment_source,zelle_receipt_url,1,seating_capacity);
                                            sendResponse.sendSuccessData({},constant.responseMessage.SUCCESS, res, 200);
...
const bookTable = (dbName,branch_id,user_id,
    ...
    amount,payment_source,reciept_url,payment_type,seating_capacity)=>{
    return new Promise(async(resolve,reject)=>{
    ...
    try {
        ...
        if(parseInt(isProductOffer)>0){
            await updateSupplierOffer(supplierId,
                offerValue,isProductOffer,req.dbName);
...
const updateSupplierOffer = async (supplierId, offerValue,
   ...
   try {
           ...
           is_products_offer = ?,
            offerValue = ? where id=? `;
           let params = [isProductOffer,offerValue,supplierId];
           await ExecuteQ.Query(dbName,query,params);
