...
const Add = async (req, res) => {
    ...
    logger.debug("=======requestParams=====>>", req.params);
    let supplier_id = params &&  params.supplier_id != undefined && params.supplier_id !== "" ? params.supplier_id : req.supplier.supplier_id;
    let agent_category_id = params.agent_category_id != undefined && params.agent_category_id !== "" ? params.agent_category_id : 0;
    try {
        let params = req.body, password;
           ...
           if(settingDataKeys.keyAndValue.addDocumentsInAgent === true){
                if (req.files && req.files.addAgentDocument) {
                    for(let i=0; i<req.files.addAgentDocument.length; i++){
                        let fileName = req.files.addAgentDocument[i].name;
                    ...
                    params.country_code,params.iso,password,
                     params.offset, params.name, params.phone_number, params.email,
                      params.area_id, supplier_id, params.experience, params.occupation,
                       ...
                       agent_category_id,assigned_id,delivery_company_id,

                       drivingLicenseUrl,
                       drivingLicenseBackUrl,
                       vehicleRegisterationUrl,
                       vehicleRegisterationBackUrl,driver_license_number
);
                var userOb = {
                    id: agentId,
                    email: params.email,
    ...
    offset, name, phone_number, email, area_id, supplier_id,
     ...
     vehicleRegisterationUrl,vehicleRegisterationBackUrl,driver_license_number) {
       ...
       offset, name, phone_number, email, area_id, supplier_id,
        experience, occupation, supplier_name, commission, AgentConnection,
        adminId,agent_category_id,assigned_id,delivery_company_id,drivingLicenseUrl,drivingLicenseBackUrl,
        vehicleRegisterationUrl,vehicleRegisterationBackUrl`,employee_id,
...
// function SetAvailblty(weeks_data,dbName,agent_id){
//         var data=[]
//         logger.debug("======week_data==FTTT==",weeks_data,dbName,agent_id);
//         _.each(weeks_data,function(i){
//             logger.debug("======I===NAME==",i)
//                 _.each(i.avail_time,function(j){
//                     logger.debug("======J===NAME==",j)
//                     data.push(parseInt(agent_id),i.day_id,j.start_time,j.end_time)
//                 })  
//         })
//         var finaData=chunk(data,4);
//         logger.debug("=======FINAL-DATA==",finaData);
//         return new Promise((resolve,reject)=>{
//             var sql ="insert into agent_time (`agent_id`,`week_day_id`,`start_time`,`end_time`) values ?"
//              multiConnection[dbName].query(sql,[finaData],function(err,data){
//                     if(err){
//                         reject(err)
//                     }
//                     else{
...
const ListSupplierAgent = async (req, res) => {
    try {
        ...
        var AgentData = await AgentListForSupplier(AgentConnection, req.supplier.supplier_id, 
            search, limit, offset, order, startDate, endDate)
...
function AgentUpdate(employee_id,agent_commission_type,
    ...
    return new Promise((resolve, reject) => {
        ...
        var updateQuery = `update cbl_user set employee_id=?,country_code=?,iso=?,supplier_name=?,name=?,
        ...
        var st = AgentConnection.query(updateQuery, [employee_id,country_code,iso,supplier_name,
...
async function RunTimeAgentConnection(data) {

var password=await universalFunc.getDecryptData(data.password)
...
const BookingAssignmnt = async (req, res) => {
    try {
        ...
        let cartId=await Execute.Query(req.dbName,"select cart_id from orders where id=?",[params.orderId]);
        if(cartId && cartId.length>0){
            ...
            let ads_on_data=await Execute.Query(req.dbName,ads_on_query,[cartId]);
...
const ListAccToArea = async (req, res) => {
    try {
        ...
        // logger.debug("==DATABASE==",GetAgentDbData);
        var AgentConnection = await RunTimeAgentConnection(GetAgentDbData);
        var AgentData = await AgentListAccToArea(AgentConnection, req.query.areaId,
...
const addCategoriesForAgent = async (req, res) => {
    try {
        ...
        let query = "insert into cbl_user_categories (name,type) values(?,?)";
        let params = [param.name,param.type]
        await Execute.QueryAgent(AgentConnection,query,params);
