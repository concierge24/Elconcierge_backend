...
function TotalOrderRevenue(dbName, start_date, end_date, supplierId,supplierBranchId=0) {
    return new Promise(async (resolve, reject) => {
        ...
        var count = 0;
        var sql = "SELECT IFNULL(DAYOFWEEK(`created_on`),0) as week_day,IFNULL(DATE(`created_on`),0) as created_at,IFNULL(SUM(net_amount),0) as total_revenue from orders where DATE(created_on) >=? and DATE(created_on) <=? and supplier_branch_id = ? and status = 5 "
        let result1=await ExecuteQ.Query(dbName,q,[supplierId]);
        // multiConnection[dbName].query(q, [supplierId], function (err, result1) {
            var sql = "SELECT IFNULL(DAYOFWEEK(`created_on`),0) as week_day,IFNULL(DATE(`created_on`),0) as created_at,IFNULL(SUM(net_amount),0) as total_revenue from orders where DATE(created_on) >=? and DATE(created_on) <=? and supplier_branch_id = ? and status = 5 "
            if (result1.length) {
...
const latestPendingOrdersV2 = async function (db_name, supplierId, limit,supplierBranchId=0) {
    var results = [];
    return new Promise(async (resolve, reject) => {
        var sql = "select o.self_pickup,IFNULL(odp.discountAmount,0) as discountAmount,o.net_amount,o.payment_type, o.referral_amount,crt.area_id,sb.supplier_id,o.is_agent,o.id,o.created_on,o.schedule_date,s.name as supplier,o.status,u.email As User_Name,u.mobile_no " +
            "from orders o join supplier_branch sb on sb.id=o.supplier_branch_id join supplier s on sb.supplier_id=s.id join cart crt on crt.id=o.cart_id " +
...
const latestActiveOrdersV2 = async function(db_name,supplierId,limit,supplierBranchId){
    ...
    return new Promise(async (resolve,reject)=>{
        try{
        var sql="select o.self_pickup,crt.area_id,o.referral_amount,IFNULL(odp.discountAmount,0) as discountAmount,sb.supplier_id,o.net_amount,o.is_agent,o.id,o.created_on,o.schedule_date,s.name as supplier,o.status,u.email As User_Name,u.mobile_no " +
        "from orders o join supplier_branch sb on sb.id=o.supplier_branch_id join supplier s on sb.supplier_id=s.id  join cart crt on crt.id=o.cart_id "+
