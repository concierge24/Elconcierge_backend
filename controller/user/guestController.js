...
const getSecreteDbKey=async (req,res)=>{
 ...
 async function getSecret(domain){
    return new Promise(async(resolve,reject)=>{
        ...
        cblConnection.query(sqlSt,[domain],async function(err,data){
            if(err){
                reject(err)
            }
            else{
                if(data && data.length>0){
                    ...
                    var crypted=await universalFunc.getDecryptData(data[0].name)
                    resolve(crypted)
                }
                else{
                    resolve("")
...
function gettSetting(dbName){
 ...
 return new Promise(async (resolve,reject)=>{
    try{
        let data=await ExecuteQ.Query(dbName,"select `key`,`value` from tbl_setting",[]);
...
const encrypt=async(req,res)=>{
    let _encrypt=new universalFunc.encrypt(req.body.dbName);
    let _key=await _encrypt.data();
    sendResponse.sendSuccessData({enryptedDb:_key}, constant.responseMessage.LOGGED_IN, res, constant.responseStatus.SUCCESS);
}
...
const getChat  = async(request, res) =>{
	try{
        ...
        var DbName=await universalFunc.getDecryptData(request.headers.secretdbkey)
        ...
        if(data.order_id && data.order_id!=""){
		 ...
		 if(data.userType == constant.TYPE.ADMIN) {
            ...
            var db_name=await universalFunc.getDecryptData(request.headers.secretdbkey)
...
const getSubCategoryOfParent=async (req,res)=>{
    try{
        ...
        if(search==0){
            ...
            IF((select count(*)  from product  where product.sub_category_id=c.id) > 0, 1, 0) as is_product,
    ...
    let  user_created_id =  randomize('A0', 30);
        try{
            var sql = ` insert into supplier(name,email,mobile_number_1, 
                ...
                license_end_date,license_document)values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`;
...
const savePushNotificationOfAgent = async(req,res)=>{
    try{
        ...
        let params2 = [data[0].user_id, data[0].supplier_id, order_id, message, status,1]
