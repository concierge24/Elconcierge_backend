...
const newRegistration=async (req,res)=>{
    ...
    let details={};
        try{
            ...
            logger.debug("==APPLE==RESULT=",result)
...
const getMyReferralData=async (req,res)=>{
        try{
            logger.debug("==========Ente==")
            let referredUsersByMe=await Execute.Query(req.dbName,`select ur.receive_price,ur.given_price,us.user_image,us.firstname,us.lastname,us.email,us.mobile_no,us.country_code from user
...
const newRegistrationuser=async (req,res)=>{
    ...
    // let details={};
        try{
            ...
            await checkEmailAlreadyExist(req.dbName,email);
            await checkPhoneAlreadyExist(req.dbName,mobileNumber)

                if (req.files && req.files.profilePic) {
                    image = await uploadMgr.uploadImageFileToS3BucketNew(req.files.profilePic);
                //    image = await uploadProfilePic(req.files.profilePic,res);
               }
               ...
               if(Object.keys(twilioata).length>0){
                ...
                let insertId = await saveUser(req.dbName,referralCode_1,
                    ...
                    longitude,password,accessToken,0,user_created_id,
                    countryCode,mobileNumber,otp,image,first_name,abn_number,business_name,dateOfBirth);

                // await referralCodeCheck(req.dbName,referralCode,save_user_result.id)
                    ...
                    // emailTemp.userRegister(req,res,AdminMail,details.firstname,'',details.email,languageId,dbConnection,function(err,result){
                    //     if(err){
                    //         console.log("..****register email*****....",err);
                    //     }
                    // })

            sendResponse.sendSuccessData({access_token:accessToken}, constant.responseMessage.SUCCESS, res, 200);
    ...
    let isAlreadyRegistered=0
        try{
            ...
            if(phoneData && phoneData.length>0){
                await Execute.Query(req.dbName,"update `user` set otp=?,device_token=?,device_type=?,latitude=?,longitude=? where id=?",[otp,deviceToken,deviceType,latitude,longitude,phoneData[0].id])
            ...
            let insertedId = await saveUserData(req.dbName,myReferralCode,email,deviceToken,deviceType,latitude,
                ...
                countryCode,mobileNumber,otp,"",name);
                userId=insertedId
            }
            phoneData=await Execute.Query(req.dbName,"select customer_payment_id,squareup_cust_id,braintree_customer_id,peach_customer_id,user_created_id,id,otp,country_code,mobile_no from `user` where id=?",[userId]);
            let twilioata=await Universal.getTwilioData(req.dbName);
...
const referralCodeCheck = (dbName,referralCode,user_id)=>{
    return new Promise(async(resolve,reject)=>{
...
const saveUserData = (dbName,referralCode,email,device_token,device_type,
    latitude,longitude,password,accessToken,otp_verified,user_created_id,
    country_code,mobile_no,otp,image,name)=>{
    return new Promise(async(resolve,reject)=>{
        try{
            let query = "insert into user (braintree_customer_id,is_sweetoo,loyalty_points,email_verified,is_logged_in,area_id,fb_access_token,gender,lastname,phone_no,is_deleted,is_verified,old_mobile_no,is_active,referral_id,email,device_token,device_type,latitude,longitude,password,access_token,otp_verified,user_created_id,country_code,mobile_no,otp,user_image,firstname)values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
            let params = ["",0,0,1,0,0,"",0,"",mobile_no,0,0,"",0,referralCode,email,device_token,device_type,latitude,longitude,password,accessToken,otp_verified,user_created_id,country_code,mobile_no,otp,image,name];
            let result = await Execute.Query(dbName,query,params);
            resolve(result.insertId)
        }catch(err){
            logger.debug(err)
...
const checkEmailAlreadyExist = (dbName,email_id)=>{
    return new Promise(async(resolve,reject)=>{
        let query = "select * from user where email=? & is_deleted=0"
        let params = [email_id]
        let result = Execute.Query(dbName,query,params);
...
const checkPhoneAlreadyExist = (dbName,mobile_no)=>{
    return new Promise(async(resolve,reject)=>{
        let query = "select * from user where mobile_no=? and is_deleted=0"
        let params = [mobile_no]
        let result = Execute.Query(dbName,query,params);
        if(result && result.length){
            reject("User already registered with this Mobile Number.")
...
const uploadDocument=async (req,res)=>{
    try{
        ...
        if(req.files.documents){
                ...
                let _document=new  model.users.document(req.users.id,documentString,_type);
...
const categorySuppliers = async (dbName,
    ...
    return new Promise(async(resolve,reject)=>{
        try{
            ...
            sql += " ctg.name as category_name, sml.address,sml.name,s.logo,st.is_open as status,st.start_time,st.end_time," +
...
class Phone {
 ...
 class registeration {
    /**
     * @description used for complete an registeration by last step
     * @param {*Object} req 
     * @param {*Object} res 
     * @param {*Function} next 
     */
    static async stepThird(req,res,next)
    {
        try{
            let _accessToken=req.headers["authorization"] || req.body.accessToken;

            let _setCriteria={
                where:{
                    key:["ride_registeration","ride_base_url","ride_db_secret_key"]
                }
            };
            let  _name=req.body.name;
            let  _businessName=req.body.business_name;
            let  _dateOfBirth=req.body.dateOfBirth || "0000-00-00";
            let  _idForInvoice=req.body.id_for_invoice;
            let  _abnNumber=req.body.abn_number;
            let  _licenseNumber=req.body.license_number || "";
            ...
            let  _details=new model.users.details(0,_accessToken,_email);
            let  _detailsByToken=await _details.token();
            _userImage=_detailsByToken.user_image;
            if (req.files && req.files.profilePic) {
                _userImage  = await uploadMgr.uploadImageFileToS3BucketNew(req.files.profilePic);
            }
            _detailsByToken.user_image=_userImage;
            let  _detailsUpdate=new model.users.details(0,_accessToken,_email,_name,_idForInvoice,_businessName,_dateOfBirth,_userImage,_abnNumber,_licenseNumber);
            await _detailsUpdate.updateData();
            let _rideInit=new model.users.ride(_detailsByToken.country_code,_detailsByToken.mobile_no,_detailsByToken.password,_detailsByToken.email,_detailsByToken.access_token,_detailsByToken.device_token,_detailsByToken.device_type,_name,_detailsByToken.mobile_no,_detailsByToken.user_image,_detailsByToken.latitude,_detailsByToken.longitude,_isRideRegEnable,_rideApiInfo);
            await _rideInit.signLogin();
            await sequelized.close();
            sendResponse.sendSuccessData(_detailsByToken, constant.responseMessage.SUCCESS, res,200);
     }
         catch (err) {
            logger.debug("==Login==Err!===>>", err);
            let errTypeMsg=typeof err ==='object'?err.sqlMessage:err
