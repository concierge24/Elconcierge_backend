    ...
    async.waterfall([
            ...
            function (cb) {
                reports.orderListing(req,req.dbName,res, startLimit,
                     ...
                     is_download,user_type_id, cb);
            },
            function(list,cb){
                // logger.debug("=========list in order report=====++",list)
                if(list){
                    logger.debug("+=================1===============")
                    cb(null,list)
                }else{
                ...
                // sql += "  where DATE( o.created_on ) >= '"+startDate+"' AND DATE( o.created_on ) <=  '"+endDate+"' " +
...
exports.agentReport = async function (req, res) {
    ...
    async.waterfall([
           ...
           async function (cb) {
                ...
                agentList(agentConnection,res, limit, offset,supplierId,startDate,endDate,order_by,is_desc,is_admin,search,is_download, cb);
            },
            function(list,cb){
                cb(null,list)
                // final.agents = list
                ...
                //     var sql = "select cu.name,cu.email,cu.id as user_id,count(cuo.user_id) as total_orders,IFNULL(sum(commission_ammount),0) as revenue "
                //         sql += "from cbl_user cu left join cbl_user_orders cuo on cuo.user_id = cu.id and cuo.status=5 and DATE(cuo.created_on) >='"+startDate+"' "
                //         sql += "and DATE(cuo.created_on) <='"+endDate+"' group by cu.id "
                // }

                // let stmt = agentConnection.query(sql,[],function (err, result) {
                //     logger.debug("===========agent final list ===count count========",stmt.sql)
                //     if (err) {
                //         console.log("errror", err);
                //         sendResponse.somethingWentWrongError(res)
                //     }
                ...
                //         logger.debug("=============result-=====count=======",result.length)
                //         // logger.debug("===============final count======",final)
                //        cb(null,final)
                //     }
...
exports.orderListing = async function(req,db_name,res, startLimit,
     ...
     endDate,order_by,is_desc,is_download,user_type_id,callback) {
    ...
    var sql = " select u.user_type_id,o.created_on,o.id,o.status,u.id as user_id,u.email user_email,u.firstname,s.name supplier_name,  ";
    sql += "  o.order_source,o.schedule_order from orders o join supplier_branch b on  ";
    sql += "  o.supplier_branch_id = b.id join supplier s on s.id = b.supplier_id join user u on u.id = o.user_id  ";
...
async function agentList(agentConnection,res, limit, offset,supplierId,startDate,endDate,order_by,is_desc,is_admin,search,is_download, cb){
    var final = {}
    ...
    logger.debug("============supplierId after condition=======",supplierId)
    var sql = "select cu.name,cu.supplier_id,cu.supplier_name,cu.email,cu.id as user_id,count(cuo.user_id) as total_orders,IFNULL(sum(commission_ammount),0) as revenue "
    sql += "from cbl_user cu left join cbl_user_orders cuo on cuo.user_id = cu.id and cuo.status=5 and DATE(cuo.created_on) >='"+startDate+"' "
    sql += "and DATE(cuo.created_on) <='"+endDate+"' where "
    ...
    //     var sql = "select cu.name,cu.email,cu.id as user_id,count(cuo.user_id) as total_orders,IFNULL(sum(commission_ammount),0) as revenue "
    //     sql += "from cbl_user cu left join cbl_user_orders cuo on cuo.user_id = cu.id and cuo.status=5 and DATE(cuo.created_on) >='"+startDate+"' "
    //     sql += "and DATE(cuo.created_on) <='"+endDate+"' where cu.supplier_id = "+supplierId+" group by cu.id "+order+" limit ?,? "
    // }else{
    //     var sql = "select cu.name,cu.email,cu.id as user_id,count(cuo.user_id) as total_orders,IFNULL(sum(commission_ammount),0) as revenue "
    //         sql += "from cbl_user cu left join cbl_user_orders cuo on cuo.user_id = cu.id and cuo.status=5 and DATE(cuo.created_on) >='"+startDate+"' "
    //         sql += "and DATE(cuo.created_on) <='"+endDate+"' group by cu.id "+order+" limit  ?,?"
    // }
