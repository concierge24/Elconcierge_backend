...
exports.accountReceivablelist = function (req,res) {
    ...
    async.auto({
        ...
        receivableListing:['checkauthority',function(cb){
            ...
            adminAccounts.accountReceivableListing(req.dbName,res,supplier,startDate,endDate,status,search,limit,offset,is_download,country_code,country_code_type,function (err,result) {
                if(err) {
                    sendResponse.somethingWentWrongError(res);
                }
                else {
...
if(country_code!='' && country_code_type!=''){
   ...
   async.auto({
       ...
       statementDetails:async function (cb) {
           try{
            sql1='SELECT o.delivery_charges, o.payment_source,o.self_pickup,o.card_payment_id as payment_id, o.transaction_id as payment_reference_number ,o.id as id,u.firstname,o.promo_discount,o.delivered_on,o.net_amount,o.handling_admin,o.handling_supplier,o.urgent_price,' +
               ...
               ' UNION ALL ' +
               'SELECT o.delivery_charges,o.payment_source,o.self_pickup, o.card_payment_id as payment_id, o.transaction_id as payment_reference_number, o.id as id,u.firstname,o.promo_discount,o.delivered_on,o.net_amount,o.handling_admin,o.handling_supplier,o.urgent_price,' +
...
exports.driverAccountPayablelistV1 = function (req,res) {
    ...
    async.auto({
        ...
        payableListing:['checkauthority',function(cb){
            ...
            adminAccounts.driverAccountPayableListingV1(req.dbName,res,agentId,supplier,startDate,endDate,status,search,limit,offset,is_download,country_code,country_code_type,function (err,result) {
                if(err) {
                    sendResponse.somethingWentWrongError(res);
                }
                else {
...
exports.driverAccountPayableListingV1 = async function(db_name,res,agentId,supplier,startDate,endDate,status,search,limit,offset,is_download,country_code,country_code_type,callback) {
    ...
    if(status && status == "1"){
        ...
        async.auto({
            ordersPart:async function (cb) {
                 ...
                 sql="SELECT (SELECT request_status from  "+db_name+".agent_supplier_payouts where order_id=user_orders.id limit 1) as withdraw_request_status,round(("+mUnit+" * acos( cos( radians( supplierinfo.latitude) ) * cos( radians( user_address.latitude ) ) * cos( radians( user_address.longitude ) - radians(supplierinfo.longitude) ) + sin( radians(supplierinfo.latitude) ) * sin( radians( user_address.latitude ) ) ) )) as distance, user_orders.payment_source, user_orders.card_payment_id as payment_id, user_orders.transaction_id as payment_reference_number, IF( capo.status ='2', 'Partially Paid', IF( capo.status = '1', 'Paid', 'Not Paid' ) ) payment_status,capo.transaction_mode , cuo.`order_id`, cuo.customer_name, cuo.customer_phone_number, cuo.customer_email, cuo.customer_id, cuo.`commission_ammount`,cuo.agent_base_price,cuo.agent_delivery_charge_share, cuo.`user_id`, cuo.supplier_id, cuo.supplier_name, cu.name, cu.email, cu.phone_number,cu.id as agent_id,cu.country,cu.city,cu.state, IF( cu.agent_commission_type=0, 'Percentage', 'Flat' ) agent_commission_type, (cuo.`net_amount`-cuo.`promo_discount`) as net_amount, cuo.`delivery_charges`, cuo.`waiting_charges`, cuo.`tip_agent`, cuo.`status`, cuo.delivered_on delivery_date, cuo.`card_payment_id`, cuo.`payment_type`,IF( TIMESTAMPDIFF(MINUTE,cuo.shipped_on,cuo.delivered_on) > 0, TIMESTAMPDIFF(MINUTE,cuo.shipped_on,cuo.delivered_on), '0' ) as duration, (cuo.`commission_ammount` + cuo.`tip_agent` + cuo.agent_base_price + cuo.agent_delivery_charge_share) as total_amount FROM `cbl_user_orders` cuo join cbl_user cu on cuo.user_id=cu.id left join cbl_account_payable_order capo on cuo.order_id=capo.order_id join "+db_name+".orders as user_orders on cuo.order_id=user_orders.id left join "+db_name+".user_address user_address on user_orders.user_delivery_address=user_address.id left join "+db_name+".supplier supplierinfo on cuo.supplier_id=supplierinfo.id WHERE cu.supplier_id='0' AND DATE( cuo.delivered_on ) >= '"+startDate+"' AND DATE( cuo.delivered_on ) <=  '"+endDate+"' "+status_check+" AND (cuo.customer_name LIKE '%"+search+"%' OR cuo.order_id LIKE '%"+search+"%') AND cuo.user_id LIKE '%"+agentId+"%' AND cuo.supplier_id LIKE '%"+supplier+"%' "+country_code_query+" order by cuo.order_id + 0 desc LIMIT "+offset+","+limit;
                 console.log("sql #########$$$$$$$$$$$$$$$$$$$$$$$$$",sql)
...
exports.driverStatement = function (req,res) {
    ...
    async.auto({
        ...
        getStatement:['checkauthority',function(cb){
            ...
            adminAccounts.getDriverStatement(req.dbName,res,agentId,supplier,startDate,endDate,search,limit,offset,is_download,country_code,country_code_type ,function (err,result) {
...
exports.getDriverStatement=async function(db_name,res,agentId,supplier,startDate,endDate,search,limit,offset,is_download, country_code,country_code_type,callback) {
    var data={};
    var credit=0;
    var debit=0;
    var balance=0;
    ...
    exports.payoutAgentRequestList = function (req,res) {
        ...
        async.auto({
            getAllPayouts:async function(cb) {
                var sql="select asp.*, (SELECT name from "+req.dbName+"_agent.cbl_user where id=asp.agent_supplier_id) as agent_name from agent_supplier_payouts asp where type='1' "+query1+" order by asp.id desc LIMIT "+offset+","+limit;
                var result = await ExecuteQ.Query(req.dbName,sql)

                var sqlCount="select count(asp.id) as cnt from agent_supplier_payouts asp where type='1' "+query1;
...
function agentPayoutPayments(dbName,res,data,callback) {
    ...
    function supplierPayoutPayments(dbName,res,data,callback) {
        var id=0;
        console.log("111111111111111111111")
        var unsuccessfulOrderIds = [];
            async.auto({
