...
exports.orderListingV2 = function (req, res) {
    ...
    async.auto({
        ...
        orderList: ['blankField', 'authenticate', 'checkauthority', function (cb) {
            ...
            loginFunctions.adminOrdersV2(req.dbName,res,limit,offset,serachText,serachType,tab_status,sub_status,
                payment_type,
                start_date,
                end_date,
                country_code,
...
exports.confirmPendingOrder = async function (req, res) {
    ...
    async.auto({
            ...
            pendingOrder: ['blankField', 'authenticate', 'checkauthority','checkOrderStatus', function (cb) {
            ...
            sendUserAgentNotification: ['sendPushNotification', async function (cb) {
                ...
                var getAgentDbData=await common.GetAgentDbInformation(req.dbName);
                var agentConnection=await common.RunTimeAgentConnection(getAgentDbData);
                var sqlQuery="select cbl_user_orders.*,cbl_user.device_token,cbl_user.language_id from cbl_user_orders join cbl_user on cbl_user.id=cbl_user_orders.user_id where order_id=?";
                ...
                if(user_order_data && user_order_data.length>0){
                    ...
                    let agent_language_data=await Execute.QueryAgent(agentConnection,`select language_code,id from language where id=?`,[parseInt(user_order_data[0].language_id)]);
                    req.userLanguage=agent_language_data && agent_language_data.length>0?agent_language_data[0].language_code:"en"
                    notificationLanguage=agent_language_data && agent_language_data.length>0?parseInt(agent_language_data[0].id):14
                };
            ...
            if(status == 1){
                ...
                emailTemp.acceptOrder(self_pickup,req,req.dbName,res,'',userName,net_amount,created_on,schedule_date,orderId,supplierName,supplierName,payment_type,userEmailId,notificationLanguage,function(err,result){
                    if(err){
                ...
                emailTemp.orderRejections(self_pickup,req,req.dbName,res,'',userName,net_amount,created_on,schedule_date,orderId,supplierName,supplierName,payment_type,userEmailId,notificationLanguage,function(err,result){
                    if(err){
