 ...
 * ------------------------------------------------------
 */
exports.authenticateAccessTokenSupplier = function (dbName,accesstoken, res, callback) {
...
module.exports.sendMailthroughSMTP = async function (smtpData,res,subject,
    ...
    if(objectLength>0){
        ...
        if(smtpData.hasOwnProperty("smtp_host") && smtpData.hasOwnProperty("smtp_port")){
            logger.debug("=====ENTING==>>",smtpData.hasOwnProperty("smtp_host"))
...
// setup e-mail data with unicode symbols
    logger.debug("======Mail=Option======",mailOptions)
...
module.exports.getrecieverSupplierAdminDetails =  async (user_created_id, type ,dbName)=>{
    return new Promise(async (resolve,reject)=>{
        try{
        var sql = "select * from supplier where user_created_id =?  and is_deleted=?  ";
        if(type=="4"){
            sql = "select * from admin where user_created_id =?";
        }
        let data=await ExecuteQ.Query(dbName,sql,[user_created_id,0])
        return resolve(data)
    }
    catch(Err){
...
module.exports.getrecieverSuperAdminDetails =  async (dbName)=>{
    return new Promise(async (resolve,reject)=>{
        try{
        sql = "select * from admin where is_superadmin =?";
