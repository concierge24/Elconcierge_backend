...
exports.orderDescriptionV2= function(dbName,res,orderId,callback) {
    ...
    async.auto({
      orderDetails:function (cb) {
          var sql="select o.order_source,o.is_dine_in,s.local_currency,s.currency_exchange_rate,o.loyality_point_discount,o.have_coin_change,o.table_id,o.slot_price, o.wallet_discount_amount,o.is_schedule,o.schedule_end_date,op.handling_admin as item_handling,op.id as orderPriceId,(p.quantity-p.purchased_quantity) as left_quantity,o.is_edit,o.edit_by,s.delivery_min_time,ct.terminology,o.type,o.user_service_charge,s.id as supplier_id,o.supplier_branch_id,o.pres_description,o.pres_image1,o.pres_image2,o.pres_image3,o.pres_image4,o.pres_image5,crt.addOn,op.product_id,crt.id as cart_id,crt.questions,IFNULL(op.duration,0) as wduration,IFNULL(odp.discountAmount,0) as discountAmount,odp.promoCode,sb.branch_name,sb.address as branch_address,u.id as user_id,u.user_image,p.pricing_type,o.cart_id,o.tip_agent,o.payment_source,o.zelle_receipt_url,o.self_pickup,o.id,o.created_on, o.transaction_id, o.card_payment_id,o.delivered_on,o.schedule_date,op.product_id,op.product_name as product,s.name as supplier,op.price as Product_cost,o.net_amount as order_cost,o.delivery_charges," +
              ...
              "join user u on o.user_id=u.id left join user_address ua on ua.id=o.user_delivery_address left join order_promo odp on odp.orderId = o.id left join cart crt on crt.id=o.cart_id join supplier_branch_product sp on sp.product_id =op.product_id join categories ct on ct.id =sp.category_id where o.id = ?";
          let stmt = multiConnection[dbName].query(sql,[orderId],async function (err,desc) {
...
function getAgentData(agentConnection,orderId,dbName){
    ...
    return new Promise(async (resolve,reject)=>{
        ...
        " ors.microblink_verification_front_url,ors.microblink_verification_back_url, ors.order_left_reason,ors.left_with_picture_url,usr.agent_bio_url,ors.signature_image_url, usr.state,usr.phone_number,usr.email, usr.agent_created_id "+
...
exports.orderNearby=function(dbName,res,orderid,status,offset,service_type,callback) {
   ...
   async.auto({
       ...
       calculateWaitingCharges:['getStatus',async function(cb){
        try{
           let sql = "SELECT TIMESTAMPDIFF(MINUTE,preparation_pickup_date_time,NOW() )  as extraTime from orders where id=?";
            let result=await ExecuteQ.Query(dbName,sql,[orderid]);
...
exports.deliveredOrder=function (dbName,res,orderid,status,offset,service_type,callback) {
    ...
    async.auto({
        ...
        updateStatus:['checkStatus',async function (cb) {
            ...
            // if(orderStatus == 11){
                ...
                // multiConnection[dbName].query(sql,[status,date,date,date,orderid],function(err,result) {
                //     if (err) {
            ...
            // }
            // if(orderStatus == 3){
            //     logger.debug("......",status,date,orderid);
            //     var sql= 'update orders set status=?,delivered_on =?,near_on =? where id=?';
            //     multiConnection[dbName].query(sql,[status,date,date,orderid],function(err,result) {
            //         if (err) {
            //             // multiConnection[dbName].rollback(function () {
            ...
            // }
            // if(orderStatus == 5){
            //     console.log("......",status,date,orderid);
            //     var sql= 'update orders set status=?,delivered_on =? where id=?';
            //     multiConnection[dbName].query(sql,[status,date,orderid],function(err,result) {
            //         if (err) {
            //             // multiConnection[dbName].rollback(function () {
        ...
        getData:['updateStatus',async function (cb) {
            ...
            var sql1='select o.user_id,o.user_service_charge,o.refund_amount,o.tip_agent,supplier_commision,o.payment_type,o.net_amount,o.handling_admin,o.handling_supplier,s.id as supplier,a.id as admin, ' +
                ' o.slot_price,o.delivery_charges,o.card_payment_id from orders o join supplier_branch sb on sb.id=o.supplier_branch_id ' +
    ...
    //     getCategory:['getData',function (cb) {
    //         var sql6= 'select op.quantity,op.price,p.category_id from orders o join order_prices op on o.id = op.order_id join product p on p.id =op.product_id  ' +
    //             'where o.id = ? ';
    //         multiConnection[dbName].query(sql6,[orderid],function(err,reply)
    //         {
    //             if(err) {
    //                 multiConnection[dbName].rollback(function () {
    //                     console.log('err2-----',err);
    ...
    //         });
    //     }],
    //     getcommission:['getCategory',function (cb) {
    //  /*       var sql6='select op.price,p.commission_type,p.commission from orders o join order_prices op on op.order_id=o.id join product p on ' +
    //             'p.id=op.product_id where o.id=?';
    //    */
    //         cate=cate.toString();
    //    var sql6= 'select category_id,commission_type,commission from supplier_category where supplier_id =? and category_id IN ('+cate+') group by category_id'
    //         multiConnection[dbName].query(sql6,[supplierId],function (err,comm) {
    //             console.log(".......................getcommsiioon..................",err,comm);
    //             if(err){
    //                     multiConnection[dbName].rollback(function () {
    ...
    //                                 (function (j) {
    //                                     if(product[j].category_id = comm[i].category_id){
    //                                         if(comm[i].commission_type == 0) {
    //                                             com=com+comm[i].commission;
    //                                         }
    //                                         else {
    //                                             com=com+((comm[i].commission * (product[j].price*product[j].quantity)) / 100);
    //                                         }
    //                                         if((i==comm.length-1) && (j==product.length-1)) {
    ...
    //     }],
        getAccountData:['promoDiscount',async function(cb){
          ...
          if(method == 1) {
            ...
            //   });
                ...
                let check=await ExecuteQ.Query(dbName,sql2,[supplierId, date1])
                // multiConnection[dbName].query(sql2, [supplierId, date1], function (err, check) {
                //     console.log(".......................getcommsiioon...........method 2.......",err,check);
        ...
        insertion1:['getAccountData',async function (cb) {
         ...
         if(method == 1){
            ...
            payAmount=isNaN(payAmount-(bear_by_supplier!=0?discount_amount:0))==true?0:(payAmount-(bear_by_supplier!=0?discount_amount:0));
             ...
             if(length){
                 if(status1==1){
                    ...
                    //      logger.debug("===========update account_payable=====1=====",stmt.sql3)
                    //      if (err) {
                    //         //  multiConnection[dbName].rollback(function () {
                    ...
                    //     logger.debug("===========update account_payable=====*****=====",stmt.sql3)
                    //      if (err) {
                    //         //  multiConnection[dbName].rollback(function () {
                 ...
                 logger.debug("==============insert into account payable=========",adminId,supplierId,payAmount,0,payAmount,0,date)
            ...
            // payAmount=handling_admin+supplier_commision+user_service_charge+(parseInt(is_agent_of_supplier)==0?agent_commision:0)+(parseInt(is_agent_of_supplier)==0?tip_agent:0);
            // logger.debug("=payAmount=bear_by_supplier==discount_amount==",payAmount,bear_by_supplier,discount_amount)
            // payAmount=isNaN(payAmount-(bear_by_supplier==0?discount_amount:0))==true?0:(payAmount-(bear_by_supplier==0?discount_amount:0));
            // logger.debug("====agent_commision=is_agent_of_supplier==payAmount=>",agent_commision,parseInt(is_agent_of_supplier),payAmount);
            
            payAmount=amount-(handling_admin+user_service_charge+supplier_commision+(parseInt(is_agent_of_supplier)==0?agent_commision:0)+(parseInt(is_agent_of_supplier)==0?tip_agent:0)+(parseInt(is_agent_of_supplier)==0?waiting_charges:0));
            payAmount=isNaN(payAmount-(bear_by_supplier!=0?discount_amount:0))==true?0:(payAmount-(bear_by_supplier!=0?discount_amount:0));
             ...
             if(length){
                 ...
                 let result1=await ExecuteQ.Query(dbName,sql3,[adminId,supplierId,payAmount,0,payAmount,0,date]);
                //  let stmt = multiConnection[dbName].query(sql3,[adminId,supplierId,payAmount,0,payAmount,0,date],function (err,result1) {
                //     logger.debug("===========update account_payable=====5=====",stmt.sql3)
                //      if (err) {
                //         //  multiConnection[dbName].rollback(function () {
                //              console.log('err6-----', err);
                //             cb(err);
        ...
        insertion2:['insertion1',async function (cb) {
            ...
            if(method==1){
                ...
                logger.debug(id, orderid,cardId,"--------------------",payAmount,"--------------------",payAmount,com)
        ...
        supplier_level:['insertion2',function(cb){
            ...
            calculate(dbName,orderid,supplierId,function(err,result){
                if(err){
                  ...
                  //  console.log("........................supplier_level.....................",typeof(result));
                  //  console.log("........................supplier_level.....................",result);

                    if(isNaN(result)){
                   //     console.log("********in else con=ditrioi ********");
                        total_points = 0;
                        
                    }else{
                  //      console.log("********in if con=ditrioi ********");
...
exports.payToAgentAndSupplier= async function (dbName,res,orderid,callback) {
    ...
    var charge_id, is_supplier_stripe_split_enabled, is_agent_stripe_split_enabled, agent_stripe_account_id, supplier_stripe_account_id, agent_id, supplier_payable_amount, agent_payable_amount,supplier_id,is_admin_driver;
    async.auto({
    ...
    return new Promise(async (resolve,reject)=>{
        ...
        //     }
            ...
            if(parseInt(result[0].payment_type)==4){
                await addWalletTransactionRecordDuringOrderRejection(dbName,result[0].user_id,refundableAmount,"",0,2,1,0)
            }
