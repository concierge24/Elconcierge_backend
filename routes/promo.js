            ...
            if(matchFind == true){
               ...
               if(req.body.details.length > 0 && req.body.details != undefined || null||""){
                     ...
                     for(var i =0;i<len;i++){
                        ...
                        var sql = "insert into promoCode (max_buy_x_get, buy_x_get_x_arr,  promo_buy_x_quantity, promo_get_x_quantity,  product_ids, promo_level, discount_percentage_by_admin, discount_percentage_by_supplier,  name,promoCode,maxUsers,minPrice,perUserCount,endDate,discountPrice,discountType,category,supplierId,promoType,startDate,detailsJson,promoDesc,firstTime,bear_by,commission_on, region_ids,category_ids,is_voucher )values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
                       var statement = multiConnection[req.dbName].query(sql, [max_buy_x_get, buy_x_get_x_arr,  promo_buy_x_quantity, promo_get_x_quantity, product_ids, req.body.promo_level, discount_percentage_by_admin, discount_percentage_by_supplier,  detailsTemp.name,detailsTemp.promoCode,detailsTemp.maxUser,detailsTemp.minPrice,detailsTemp.perUserCount,moment(detailsTemp.endDate).format('YYYY-MM-DD'),detailsTemp.discountPrice,detailsTemp.discountType,detailsTemp.details[i].categoryId,detailsTemp.details[i].supplierId,detailsTemp.promoType,moment(detailsTemp.startDate).format('YYYY-MM-DD'),saveData,detailsTemp.desc,req.body.firstTime,detailsTemp.bear_by,detailsTemp.commission_on, region_ids,category_ids,is_voucher], function (err, result) {
                   ...
                   var sql = "insert into promoCode (max_buy_x_get, buy_x_get_x_arr, promo_buy_x_quantity, promo_get_x_quantity,  product_ids, region_ids, promo_level, discount_percentage_by_admin, discount_percentage_by_supplier,   category_ids,  name,promoCode,maxUsers,minPrice,perUserCount,endDate,discountPrice,discountType,category,supplierId,promoType,startDate,detailsJson,promoDesc,firstTime,bear_by,commission_on,is_voucher)values(?,?,?,?,?,?,?,?,?, ?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
                  var statement = multiConnection[req.dbName].query(sql, [max_buy_x_get, buy_x_get_x_arr,  promo_buy_x_quantity, promo_get_x_quantity, product_ids, region_ids, req.body.promo_level, discount_percentage_by_admin, discount_percentage_by_supplier,  category_ids, detailsTemp.name,detailsTemp.promoCode,detailsTemp.maxUser,detailsTemp.minPrice,detailsTemp.perUserCount,moment(detailsTemp.endDate).format('YYYY-MM-DD'),detailsTemp.discountPrice,detailsTemp.discountType,0, 0,detailsTemp.promoType,moment(detailsTemp.startDate).format('YYYY-MM-DD'),saveData,detailsTemp.desc,req.body.firstTime,detailsTemp.bear_by,detailsTemp.commission_on,is_voucher], function (err, result) {
                       if (err) {
                           logger.debug("===========in the error of createPromo=========  ",statement.sql,err);
                             var msg = "db error"
                             sendResponse.sendErrorMessage(msg,res,500);
...
exports.listPromo = function(req,res) {
   ...
   if(region_ids != undefined && region_ids != null  && region_ids !="")
    ...
    async.auto({
        ...
        findPromo:['checkParameter',async function(cb){
            try{
                    if(is_all==1){
                ...
                let  result=await Execute.Query(req.dbName,sql,[offset,limit])
                data = result;
                cb(null)
            }
            catch(Err){
                        logger.debug("+============findPromo----Err--------",Err)
                        var msg = " db error"
                        sendResponse.sendErrorMessage(msg,res,500); 
            }
            
        }],
        countPromo:['findPromo',async function(cb){
            try{
                let sql ="SELECT DISTINCT(promoCode) FROM promoCode where isDeleted = 0 "+product_ids_condition+"  "+region_ids_condition+"   "+category_ids_condition+" ";
...
exports.checkPromoV1 = function(req,res){
   ...
   var crossTotalLimit;
    ...
    var categoryIds=[];
    var supplierIds=[];
    var discountType,minOrder=0, category_ids,region_ids,product_ids,promo_level, promo_buy_x_quantity, promo_get_x_quantity, 	buy_x_get_x_arr;
    let max_discount_value=0;
    var detailsJson = [];
    async.auto({
        checkParameter:function(cb){
            if(!(req.body.promoCode)) {
        ...
        checkPromo:['totalUserUsed',function(cb){
            ...
            let stmt = multiConnection[req.dbName].query(sql, [req.body.promoCode], function (err, result) {
                logger.debug("===========checkPromo=================query======",stmt.sql)
                console.log("...........................checkPromo............",err,result);
