...
logger.debug("===============in accountpayableListing function==========",search)
    ...
    async.auto({
        ordersPart:async function (cb) {
            try{
             sql1="select aro.status,aro.commission,IFNULL(odp.discountAmount,0) as discountAmount,aro.order_id,s.id,s.name,u.firstname,o.payment_source, o.card_payment_id as payment_id, o.transaction_id as payment_reference_number,o.delivered_on,o.handling_admin,o.handling_supplier,o.delivery_charges,o.urgent_price,(o.net_amount-IFNULL(odp.discountAmount,0)) as net_amount,aro.total_amount from account_receivable ar join account_receivable_order aro " +
                ...
                "AND DATE( o.delivered_on ) <=  '"+endDate+"' AND aro.status LIKE '%"+status+"%' AND (u.firstname LIKE '%"+search+"%' OR aro.order_id LIKE '%"+search+"%') order by aro.order_id desc LIMIT "+offset+","+limit+""
...
exports.accountReceivableListing=function   (req,dbName,res,supplier,startDate,endDate,status,search,limit,offset,callback) {
    ...
    async.auto({
        ordersPart:async function (cb) {
            try{
             sql1="select (SELECT request_status from agent_supplier_payouts where order_id=o.id limit 1) as withdraw_request_status, s.id,aro.status,aro.commission,aro.order_id,IFNULL(odp.discountAmount,0) as discountAmount,s.name,u.firstname,o.delivered_on,o.handling_admin,o.handling_supplier,o.delivery_charges,o.urgent_price,o.net_amount-discountAmount as net_amount,aro.total_amount from account_payable ar join account_payable_order aro " +
  ...
  /*  var sql1="select sum(debit) as total_debit,sum(credit) as total_credit from account_statement where supplier_id=?";
    ...
    async.auto({
        ...
        statementDetails:async function (cb) {
            try{
                sql1 ='SELECT o.delivery_charges,o.payment_source, o.card_payment_id as payment_id, o.transaction_id as payment_reference_number,o.id,u.firstname,o.delivered_on,o.net_amount,o.handling_admin,o.handling_supplier,o.urgent_price,' +
                'aro.commission,aro.total_amount,1 as accountType FROM account_statement acs join account_receivable_order aro on ' +
                'aro.order_id =acs.order_id join orders o on o.id = acs.order_id join user u on u.id =o.user_id  ' +
                ...
                ' UNION ALL ' +
                'SELECT o.delivery_charges,o.payment_source, o.card_payment_id as payment_id, o.transaction_id as payment_reference_number,o.id,u.firstname,o.delivered_on,o.net_amount,o.handling_admin,o.handling_supplier,o.urgent_price,' +
            ...
            if(parseInt(req.body.is_download)==1){
                ...
                let result = await ExecuteQ.Query(dbName,sql3,[supplier,supplier])
                let header = [ 
...
exports.supplierConnectWithStripe = async function (req,res) {
    ...
    async.auto({
        ...
        //ConnectWithStripe:['checkauthority',function(cb){
            ...
            stripe.oauth.token({
                ...
                if (err) {
                    console.log("66666666666666666666666666",err)
                    sendResponse.somethingWentWrongError(res);
                } else {
                    console.log("777777777777777777777777777777777",response)
                    ...
                    multiConnection[req.dbName].query(sql, [response.stripe_user_id, supplier_id], function (err, result) {
                        if (err) {
                            console.log("88888888888888888888888888888888",err)
                            sendResponse.somethingWentWrongError(res);
                        } else {
                            console.log("99999999999999999999999999999999",result)
                            data=result;
                            cb(null);
                        }
                    })
        ...
        if(err) {
            console.log("111211121112111211211121112111211121121112111211121112112")
            sendResponse.somethingWentWrongError(res);
        }else{
