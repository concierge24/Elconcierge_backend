...
exports.saveSeeupplierProfileSubTab1 = async function (req,res) {
    ...
    async.auto({
        ...
        isMultipleBranch:async function (cb) {
            ...
            // var vendor_query="select is_multiple_branch from screen_flow limit 1"
            // multiConnection[req.dbName].query(vendor_query,[],(err,data)=>{
            //     if(err){
            //         console.log("err",err);
            //         sendResponse.somethingWentWrongError(res);
            //     }
            ...
            //         if(data && data.length>0){
            //             if(parseInt(data[0].is_multiple_branch)==0){
            //                 multiConnection[req.dbName].query("update supplier_branch set delivery_radius=? where supplier_id=?",[delivery_radius,supplierId],(err,data)=>{
            //                     cb(null)
            //                 })
            //             }
            //             else{
        ...
        // updateLogo:['checkAccessToken',function (cb) {
        //     if (photoStatus == 0) {
        //         cb(null, logoUrl);
        //     }
        //     else {
        //         func.uploadImageFileToS3Bucket(res, req.files.logo, folder,function (err,result) {
        //             console.log("............uploadImageFileToS3Bucket...........",err,result);
        //             if(err){
        ...
        updateData:['isMultipleBranch',async function (cb) {
            ...
            if(getApiVersion>0){
                ...
                var sql = "update supplier  set is_scheduled=?,country_of_origin=?,is_dine_in=?,distance_value=?,description=?,brand=?,linkedin_link=?,facebook_link=?,nationality=?,speciality=?,user_request_flag=?,license_number=?,user_service_charge=?, address=?,is_sponser=?, country_code=?, iso=?, radius_price=?,self_pickup=?,phone = ?,mobile_number_1 = ?,mobile_number_2 = ?, "
                sql += " slogan_ol=?,slogan_en=?, trade_license_no = ? ,business_start_date = ?,is_recommended = ?, email = ?, "
                sql += "name = ?,latitude = ?,longitude = ?,delivery_radius = ?,is_area_restricted = ?,delivery_type = ?,commission = ?,pickup_commission=?,base_delivery_charges=?,currency_exchange_rate=?,local_currency=?,is_own_delivery=?,is_out_network=?,is_user_service_charge_flat=?,table_booking_price=?,is_free_delivery=?,table_booking_discount=? where id = ? limit 1";
               
                await Execute.Query(req.dbName,sql,[is_scheduled,country_of_origin,is_dine_in,
                    ...
                    primaryMobile,secondaryMobile,slogan_ol,slogan_en,
                    tradeLicenseNo,businessStartDate,
                    isRecommended,supplierEmail,supplierNameEn,latitude,longitude,
                    ...
                    is_free_delivery,table_booking_discount,supplierId])
               ...
               let totalLengthSuppProduct=totalSupplierProduct && totalSupplierProduct.length>0?(totalSupplierProduct.length-1):0
                ...
                var sql = "update supplier set is_scheduled=?,description=?,brand=?,linkedin_link=?,facebook_link=?,nationality=?,speciality=?,user_request_flag=?,license_number=?,user_service_charge=?, address=?,is_live=?,self_pickup=?,phone = ?,mobile_number_1 = ?,mobile_number_2 = ?, "
                sql += "trade_license_no = ? ,business_start_date = ?,is_recommended = ?, email = ?, "
                sql += "name = ?,latitude = ?,longitude = ?,delivery_radius = ?,is_area_restricted = ?,delivery_type = ?,base_delivery_charges=?,is_out_network=?,is_user_service_charge_flat=?,table_booking_price=? where id = ? limit 1";
               await Execute.Query(req.dbName,sql,[is_scheduled,description,brand,linkedin_link,
                ...
                supplierNameEn,latitude,longitude,delivery_radius,is_area_restricted,delivery_type,
...
function updateSupplierBranchProducts(res,supplierId,urgentPrice,isUrgent,urgentType,callback)
{
   ...
   /* var sql = " update product_pricing set can_urgent = ?,urgent_value = ?, urgent_type = ? where product_id IN (select product_id from supplier_branch_product where supplier_branch_id IN (select id from supplier_branch where supplier_id = ? ))";
    multiConnection[dbName].query(sql,[isUrgent,urgentPrice,urgentType,supplierId],function(err,result)
    {
