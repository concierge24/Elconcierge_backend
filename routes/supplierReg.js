...
module.exports.activeOrInActiveSupplier = function (req, res) {
    ...
    async.waterfall([
        ...
        function (cb) {
            //loginCases.logInCases(res, [sectionIdFromFrontEnd], adminId, cb, filter1);
            supplierReg.activeOrInACtive(req.dbName,res, cb, status, supplier, adminId);
        }
    ], function (error, response) {
        if (error) {
            console.error(error);
...
module.exports.regSupplierDirectlyV2 = async function (req, res) {
    ...
    var categoryIds = req.body.categoryIds;
    var supplierName = req.body.supplierName;
    let supplierNameOl = req.body.supplierNameOl
    var supplierAddress = req.body.supplierAddress;
    var supplierMobileNo = req.body.supplierMobileNo;
    var supplierEmail = req.body.supplierEmail;
...
module.exports.activeOrInACtive = function (dbName,res, callback, status, supplierId, adminId) {
    var sql = "update supplier s join supplier_admin sa on s.id = sa.supplier_id set s.is_live=?,s.is_active = ?,sa.is_active = ? where s.id IN ("+supplierId+") ";
    multiConnection[dbName].query(sql, [status,status,status], function (err, reply) {
        if (err) {
            console.error(err);
            sendResponse.somethingWentWrongError(res);
        } else {
