...
exports.supplierLoginUsingPassword = function(req, res) {
	...
	async.waterfall([
		...
		async function(cb) {
			try{
				...
				// multiConnection[req.dbName].query(sql, [email], function(err, userResponse) {
			...
			//	console.log("id",id);
				if (flag == 1) {
					if(id.is_superadmin==1){
						...
						// multiConnection[req.dbName].query(sql, [supplierId], function(err, response) {
			...
			async function(cb) {
				...
				if(superAdmin){
					...
					await ExecuteQ.Query(req.dbName,sql,[accessToken,accessToken,fcmToken,supplierAdminId])
						// multiConnection[req.dbName].query(sql, [accessToken,accessToken,fcmToken,supplierId], function (err, result) {
			...
			function(accessToken1, cb) {
				if (flag == 1) {
					...
					//checkSupplierSuperAdminOrNot(res, supplierId,0, cb);
					cb(null,superAdmin)
				} else {
					accessToken = accessToken1;
					cb(null, 1);
			...
			async function(check, cb) {
				if (check) {
					...
					let query = "select (SELECT message_id FROM `chats` WHERE (`send_to_type`='ADMIN' or `send_by_type`='ADMIN') and (send_by=s.user_created_id or send_to=s.user_created_id) and (`send_to_type`='SUPPLIER' or `send_by_type`='SUPPLIER') order by c_id desc limit 1) as message_id, s.user_created_id,s.is_own_delivery, s.name from supplier s where s.id = ?";
		...
		async function(err, result) {
			if (err) {
				sendResponse.somethingWentWrongError(res);
					...
					let supplierSubscription = await supplierSubscriptionDetails(req.dbName,supplierId)
					logger.debug("===========================")
					let query = "select (SELECT message_id FROM `chats` WHERE (`send_to_type`='ADMIN' or `send_by_type`='ADMIN') and (send_by=s.user_created_id or send_to=s.user_created_id) and (`send_to_type`='SUPPLIER' or `send_by_type`='SUPPLIER') order by c_id desc limit 1) as message_id, s.user_created_id,is_own_delivery, s.name from supplier s where s.id = ?";
					let details = await Execute.Query(req.dbName,query,[supplierId]);
...
const supplierSubscriptionDetails = (dbName,supplier_id)=>{
	return new Promise(async(resolve,reject)=>{
		let query = "select * from supplier_subscription where supplier_id=? and status = ?"
