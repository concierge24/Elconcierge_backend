...
exports.connectSocket = function (server) {
    ...
    socket.on('connection', async function(socket){
        ...
        if( socket && socket.handshake.query &&  socket.handshake.query.secretdbkey && socket.handshake.query.id  && socket.id ){
            ...
            var dbName=await universalFunc.getDecryptData(secretdbkey)
            ...
            socket.on('join_room', async (data,cb) => {
                ...
                console.log(room_id," &&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&=== room_id, name===",name)
                ...
                cb({"status":400,"message":"success","data":{"room_id":room_id}});
            ...
            socket.on(CONSTANT.SERVER.SOCKET.CHAT.SEND_MESSAGE, async (data,cb) => {
                ...
                // var socketIdToSend = "";
                // if(socket.handshake.query.userType==1){
                //     socketIdToSend =   await  AgentCommon.getagentSocketDetails(agent_connection, socket.handshake.query.id);                
                // }else if(socket.handshake.query.userType==2){
                //     socketIdToSend =   await  commonFunctions.getrecieverSocketDetails(socket.handshake.query.id,dbName);
                // }
                // logger.debug("=================socketIdtosend=========",socketIdToSend)
                ...
                if(data.detail.type  == constant.TYPE.AGENT){
                    ...
                    send_to_type="ADMIN"
                    console.log("$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$")
                    receieverDetails =  await  commonFunctions.getrecieverSuperAdminDetails(dbName);
                ...
                if(                    
                    ...
                    }else if(data.detail.message_id && data.detail.message_id!=""){
                        message_id = data.detail.message_id

                    }
                }
                data.detail.message_id = message_id;
                console.log("zzzzzzzzzzzzzzzzzzzzzzzzzzz",message_id)
                let insertMsgDetails =  await commonFunctions.insertMsgText(dbName,receiver_created_id,
                    ...
                    data.detail.thumbnail, data.detail.chat_type, data.detail.order_id,message_id,send_by_type,send_to_type);
                ...
                if ((receieverDetails[0] && receieverDetails[0].socket_id) || (socket.room!=undefined && socket.room!="")){
                    ...
                    if(
                        ...
                        //socket.broadcast.to(socket.room).emit(CONSTANT.SERVER.SOCKET.CHAT.RECEIVE_MESSAGE, data);
                        socket.broadcast.to(message_id).emit(CONSTANT.SERVER.SOCKET.CHAT.RECEIVE_MESSAGE, data);
    ...
    //   else{
    //     logger.debug('no accessToken');
    //     return socket.emit(CONSTANT.SERVER.SOCKET.ERROR.SOCKET_ERROR, CONSTANT.SERVER.SOCKET.ERROR_MSG.ACCESS_TOKEN_REQUIRED); //Not Work
    // }
    ...
    socket.on(CONSTANT.SERVER.SOCKET.START.DISCONNECT, () => {
        ...
        // middlewear.userSocketAuth(socket.handshake.query.token).then(
        //     (userData)=> {
                //    io.to(socket.id).emit("success",APP_CONSTANTS.STATUS_MSG.SUCCESS.DEFAULT);
                socket.on('on_the_way',async function(data){
                    logger.debug("===ON=THE=WAY==",data,socketInfo[data.user_id+data.client_secret_key]);
                    //for user

                    let estimatedTimeInMinutes;
                    var DbName=await universalFunc.getDecryptData(data.client_secret_key)
                    ...
                    logger.debug("=======estimatedTimeInMinutes=====",estimatedTimeInMinutes,socketInfo[data.user_id+data.client_secret_key]);
                ...
                socket.on('track_order',async function(data){
                    ...
                    logger.debug("===at time Event ==id",socket.id,socketInfo[data.access_token]);
                   if(data && data.order_id && data.secret_key  && data.access_token){
                       try{
                      
                        var db_name= await universalFunc.getDecryptData(data.secret_key)
                           ...
                           var response_data;
                           var st= agent_connections.query(sqlQuery,[data.order_id,4], async function(err,bData){
                            //    logger.debug(st.sql)
                               if(err){
                    ...
                    try{
                           ...
                           let sqlQuery =  "select utb.supplier_id,utb.branch_id as supplier_branch_id, u.id,u.latitude,u.longitude from user_table_booked utb "
                            ...
                            let tableBookingDetails = await common.getTableBookingDetails(db_name,table_booking_id);
                             logger.debug("============tablebooking details======",tableBookingDetails)
                             ...
                             logger.debug("==========getTimeadiff=====",getTimeAndDistanceForNotif)
                             ...
                             logger.debug("=============",scheduleDateTime,currentTime)
                            ...
                            let resultInMinutes = Math.round(difference / 60000);
                             ...
                             logger.debug("========",resultInMinutes,
                             notification_time,distance_of_user_supplier,notification_distance)
                             ...
                             if(parseInt(resultInMinutes)<parseInt(notification_time) && parseInt(distance_of_user_supplier)<parseInt(notification_distance)){
                                    let checkUserAlreadyInRange = await Execute.Query(db_name,
                                        ...
                                        logger.debug("========checkUserAlreadyInRange======44========",checkUserAlreadyInRange)

                                    if(checkUserAlreadyInRange && checkUserAlreadyInRange.length>0){
                                        logger.debug("========checkUserAlreadyInRange==============",checkUserAlreadyInRange)
                                        if(parseInt(checkUserAlreadyInRange[0].user_in_range)!==1){
                                            
                                            logger.debug("========checkUserAlreadyInRange=======2=======",checkUserAlreadyInRange[0].user_in_range)
